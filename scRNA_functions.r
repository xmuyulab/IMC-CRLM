...
LoadandSave_GSE178318 <- function(scRNAdataPath,savePath){
    ...
    colnames(clinical) <- sapply(colnames(clinical),function(x){return(gsub(x,pattern = '\\.',replacement = "_"))})

    barcodes <- dplyr::left_join(barcodes,clinical,"PID")

    seuobj <- CreateSeuratObject(counts = sparseExp, meta.data = barcodes, project = "CRCLM", min.cells = 3, min.features = 200)
