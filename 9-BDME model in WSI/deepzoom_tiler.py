...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--workers', type=int, default=4, help='Number of worker processes to start [4]')
    parser.add_argument('-q', '--quality', type=int, default=70, help='JPEG compression quality [70]')
    parser.add_argument('-s', '--tile_size', type=int, default=1024, help='Tile size [224]')
    ...
    if len(levels) == 2:
        out_base = os.path.join(args.ori_path, 'pyramid')

    else:
        out_base = os.path.join(args.ori_path, 'single')
    ...
    ## For testing
    # nohup python /mnt/data/lyx/IMC/WSI_HE/HE_classification_2/deepzoom_tiler.py -m 0 -b 20 -v svs -p /mnt/data/lyx/IMC/WSI_HE/WSI_Testing/SVS -f tif -c 8 > crop.log 2>&1 &
